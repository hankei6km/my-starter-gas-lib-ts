...
1. `src/main.ts` 内の `export` と `import` を変更
1. `src/index.js` 内のライセンスバナー
    - タイトルと `.zip` ファイル名を BASENAME にあわせる
1. `test/md2html.test.ts` のファイル名を `BASENAME` にあわせて変更
...
[Trusted publishing for npm packages](https://docs.npmjs.com/trusted-publishers) での手順で publish することを想定してワークフローを記述してあります。なお、Trusted publisher の設定で Environment を指定する必要があります。

## コードの記述

1. `src/${BASENAME}.ts` 内に処理を記述します
...
1. `test/xxxx.test.ts` にテストを記述します

    Jest フレームワークを利用しています。また、テストは native ESM として実行されます。

1. `test_build/index.js` にビルドされたコード用のテストを記述します

    `src/index.js` 用のテストを記述します。`src/idnex.js` とは結合された状態実行されるので `import` は行いません(結合されたファイルは `test/build/index.test.js` 上に一時的に保存される)。
